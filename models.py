...
class Wallet(db.Model):
    ...
    OOP Concepts:
    - ENCAPSULATION: Balance modification through methods
    """
    ...
    # Relationship
    user = db.relationship('User', backref=db.backref('wallet', uselist=False, lazy='joined'))
    ...
    def to_dict(self):
        """Convert wallet to dictionary for backward compatibility"""
        return {
...
class Transaction(db.Model):
    ...
    - Foreign Key: user_id -> users.id
    - Indexes on user_id, transaction_id, status for fast lookups
    - Timestamps for chronological ordering
    
    OOP Concepts:
    - ENCAPSULATION: Transaction data as structured record
    """
    __tablename__ = 'transactions'
    
    id = db.Column(db.Integer, primary_key=True)
    transaction_id = db.Column(db.String(50), nullable=False, index=True)  # TXN ID like TXN20260204215100110
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, index=True)
    username = db.Column(db.String(80))  # Username for display
    amount = db.Column(db.Float, nullable=False)
    method = db.Column(db.String(20), nullable=False)  # 'card', 'upi', 'wallet', 'netbanking'
    status = db.Column(db.String(20), nullable=False, default='success', index=True)  # 'success', 'failed'
    txn_type = db.Column(db.String(20))  # 'credit', 'debit', or None (for recharges)
    description = db.Column(db.Text)  # Transaction description
    new_balance = db.Column(db.Float)  # Balance after transaction
    txn_date = db.Column(db.String(20))  # Date string YYYY-MM-DD
    txn_time = db.Column(db.String(20))  # Time string HH:MM:SS
    ...
    def to_dict(self):
        """Convert transaction to dictionary for backward compatibility"""
        result = {
            'id': self.transaction_id,
            'user_id': str(self.user_id) if self.txn_type else self.user_id,
            'amount': float(self.amount),
            'method': self.method,
            'status': self.status,
            'description': self.description or '',
            'date': self.txn_date or (self.timestamp.strftime('%Y-%m-%d') if self.timestamp else ''),
            'time': self.txn_time or (self.timestamp.strftime('%H:%M:%S') if self.timestamp else ''),
            'timestamp': self.timestamp.isoformat() if self.timestamp else ''
        }
        if self.username:
            result['username'] = self.username
        if self.txn_type:
            result['type'] = self.txn_type
        if self.new_balance is not None:
            result['new_balance'] = float(self.new_balance)
        return result
